...
function cipher(str, offset) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (str[i] === " ") {
      ...
      if (alphabet.indexOf(str[i]) + offset > 25) {//greater than z @ 25th position (example x => 23rd position + offset of 3 => 26)
        // Q: What are you going to do? 
        ...
        encryptedStr += alphabet[alphabet.indexOf(str[i]) + offset - 26]//x => 23rd position + 3(offset) - 26 => 0
        // Q: What are you going to do? 
...
function cipherAllOffsetsAndPunc(str, offset) {
  // Q: What are you going to do? 
  // A: Create a function that takes two arguments - a string and an offset number
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (!isAlpha(str[i])) {
      encryptedStr += str[i];
      // Q: What are you going to do? 
      // A: Check if the current index of the string is not an alphabetic character

      // Q: How are you going to do it? 
      // A: with a function that checks whether the character is a member of the alphabet , the indexOf method, and a += (addition assignment) operator 

      // Q: Why are you going to do it? 
      // A: To simply add a non-alphabet character to our new string if the current index of the string is a non-alphabet character, since we don't need to encrypt it.            
    }
    else {
      // Q: What are you going to do? 
      // A: Shift the the current indexed letter in the alphabet based on the offset value given
      ...
      // Q: How are you going to do it? 
      // A: with a the modulus (%) operator

      // Q: Why are you going to do it? 
      // A: to shift the input string for all indices of the string that are not non-space characters.

      // Q: What are you going to do? 
      // A: Calculate the NEW offset into the alphabet given the offset argument

      let cypherOffset = (alphabet.indexOf(str[i]) + offset) % alphabet.length;

      // Q: What are you going to do? 
      // A: Add the shifted / encrypted letter to the new string

      encryptedStr += alphabet[cypherOffset];
    }
  }
...
console.log(cipherAllOffsetsAndPunc("I'm not hungry.", 2)) // BONUS: Write an algorithm to modify "I'm not hungry." While maintaining alphanumeric characters (case sentitivity, punctuation, symbols).
